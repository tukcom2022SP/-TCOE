...
import kotlinx.android.synthetic.main.taxi_fragment_oido.*
 ...
 */
class OidoFragment : Fragment() { //기본 탭
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.taxi_fragment_oido, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        oido_recycler.adapter = OidoRecyclerViewAdapter()
        oido_recycler.layoutManager = LinearLayoutManager(context)
    }
    ...
    inner class OidoRecyclerViewAdapter() : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

        private var oidoTaxiData : ArrayList<TaxiData> = arrayListOf()
        private var context : Context = requireContext()

        //DB에 저장된 문서를 불러와 TaxiData로 변환한 뒤 oidoTaxiData라는 리스트에 담음
        init {

            firestore?.collection("oidoTaxiShare")?.addSnapshotListener {
                ...
                if(querySnapshot != null) {
                    for (snapshot in querySnapshot!!.documents) {
                        ...
                        oidoTaxiData.add(item!!)
                    }
                    notifyDataSetChanged() //업데이트
                }
                else Log.i("TAG","querySnapshot : null")
                var sortedOidoTaxiData = oidoTaxiData.sortedBy { it.uploadTime }

                oidoTaxiData = ArrayList(sortedOidoTaxiData.reversed())
            }
        }
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            var hour = oidoTaxiData[position].departure_hour.toString().toInt()
            ...
            viewHolder.item_departure_minute.text = oidoTaxiData[position].departure_minute.toString()

            when(oidoTaxiData[position].position) {
                0 -> viewHolder.item_position.text = "정왕"
                1 -> viewHolder.item_position.text = "오이도"
            }
            viewHolder.item_entrance.text = oidoTaxiData[position].entranceNum.toString()
            viewHolder.item_current_num.text = oidoTaxiData[position].shareMember.size.toString()
            viewHolder.item_max_num.text = oidoTaxiData[position].maxNum.toString()

            if(oidoTaxiData[position].restriction == 0) {  //성별 제한이 없는 경우
            }
            else if (oidoTaxiData[position].restriction == 1 && oidoTaxiData[position].gender == "MALE") { // 동성만 제한하고 작성자가 남자인 경우
            ...
            viewHolder.setOnClickListener() {
                var dlg = taxiShareDialog(context,oidoTaxiData[position])
                dlg.showDialog()
            }
        }
        ...
        init {
            ...
            * */
            ...
            UserApiClient.instance.me { user, error ->
                ...
                Log.i("TAG", "Oido Frgment - user info: ${userId}, ${gender}")
            ...
            dialog.detail_reqBtn.setOnClickListener {
                ...
                dlg.setPositiveButton("요청") { dlg, which ->
                    ...
                    if(taxiItem.requestUser.contains(userId.toString()) //중복 요청 못하도록 함
                        ...
                        db.collection("oidoTaxiShare").document(taxiItem.docId).update(
                            "requestUser", reqUser
                        )
            ...
            dialog.detail_deleteBtn.setOnClickListener {
                ...
                dlg.setPositiveButton("삭제") { dlg, which ->
                    db.collection("oidoTaxiShare").document(taxiItem.docId).delete()
